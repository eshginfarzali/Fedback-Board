...
function getDataRender(){
    ...
    if (foundData) {
        ...
        <div class="feedback-edit-box">
          ...
          <option value="Feature" ${foundData.category === 'Feature' ? 'selected' : ''}>Feature</option>
          <option value="UI" ${foundData.category === 'UI' ? 'selected' : ''}>UI</option>
          <option value="UX" ${foundData.category === 'UX' ? 'selected' : ''}>UX</option>
          <option value="Bug" ${foundData.category === 'Bug' ? 'selected' : ''}>Bug</option>
          <option value="Enhancement" ${foundData.category === 'Enhancement' ? 'selected' : ''}>Enhancement</option>
        </select>           
          </div>
          <div class="title-text">
            <h2>Update Status</h2>
          ...
          <div class="value-text">
            ...
            <option value="Planned" ${foundData.status === 'Planned' ? 'selected' : ''}>Planned</option>
          <option value="In-Progress" ${foundData.status === 'In-Progress' ? 'selected' : ''}>In-Progress</option>
          <option value="Live" ${foundData.status === 'Live' ? 'selected' : ''}>Live</option>
    ...
    deleteBtn.addEventListener("click", () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              const updatedData = storedData.filter(item => item.id !== feddDataId);
              localStorage.setItem("remoteDataLocalStorge", JSON.stringify(updatedData));
              setTimeout(()=>{
...
function getUpload() {
    ...
    feedTitleInp.addEventListener("input", () => {
        const isValid = validateInput(feedTitleInp.value, 3, 30);
    ...
    updateFeedBtn.addEventListener("click", () => {
        ...
        if (isTitleValid && isTextValid) {
            ...
            // Your code to update feedback data
